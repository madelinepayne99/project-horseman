...
const NY = { exchangeTimezone: "America/New_York" };
...
test("today's bar after the close (plus settle grace) is NOT provisional", () => {
  // 20:45 UTC == 16:45 New York — past the 16:20 settle cutoff.
...
test("today's bar inside the post-close settle window is still provisional", () => {
  // 20:05 UTC == 16:05 New York — after the bell but inside the grace period,
...
test("timezone is taken from canonical market metadata, not assumed from the server's clock", () => {
  // Same instant, but an exchange where it is already past the close.
  const tokyo = { exchangeTimezone: "Asia/Tokyo" };
...
test("missing exchangeTimezone falls back to the US session (this phase is US-equity scoped)", () => {
...
test("an unusable timezone is treated as final rather than suppressing real data", () => {
  assert.equal(isProvisionalBar({ date: "2026-09-02" }, { exchangeTimezone: "Not/AZone" }, MIDSESSION_UTC), false);
});

test("DST is handled by real zone rules, not a fixed UTC offset", () => {
  // January: New York is EST (UTC-5), so 20:05 UTC == 15:05 — still open.
  const winterOpen = new Date("2026-01-14T20:05:00Z");
  assert.equal(isProvisionalBar({ date: "2026-01-14" }, NY, winterOpen), true);
  // Same clock time in July is EDT (UTC-4), so 20:05 UTC == 16:05 — past the bell.
  const summer = new Date("2026-07-15T20:05:00Z");
  // Inside the settle grace window, so still provisional — but for a
