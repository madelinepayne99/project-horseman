...
/**
 * TwelveDataProvider â€” real adapter over Twelve Data's /time_series endpoint.
 *
 * Free tier limits (as documented at time of writing â€” verify before
 * relying on these in production, they are Twelve Data's numbers, not ours):
 *   - 800 API calls / day
 *   - 8 API calls / minute
 *   - up to 5,000 data points per request
 *   - data delayed roughly 1â€“15 minutes depending on exchange; irrelevant
 ...
 *     on paid tiers â€” LSE-style symbols are NOT confirmed to resolve on
 ...
 * for resilience through weekends/holidays/provider gaps â€” one HTTP
 ...
 * message} â€” sometimes on a 200, sometimes on a non-2xx status. Values in
 * `values[]` are documented as STRINGS (e.g. "open": "275.23001"), which
 * schema/ohlcv.js's makeOhlcvPoint() already parses defensively.
 *
 * IMPORTANT (found via live debugging of this exact code â€” see the
 * diagnostic report for this phase): a 401/403 status code is NOT proof
 * the response actually came from Twelve Data â€” a network intermediary
 ...
 * server-side only â€” never sent to the browser â€” so a misconfigured
...
export class TwelveDataProvider extends MarketDataProvider {
  ...
  async getDailySeries(ticker) {
    ...
    if (!looksLikeTwelveData) {
      // Log server-side only â€” this is exactly the diagnostic a network/
    ...
    return makeNormalisedSeries({
      ...
      // Canonical metadata â€” translated from Twelve Data's own field names
      // here, at the adapter boundary, so nothing downstream needs to know
      // that this vendor spells it "exchange_timezone".
      exchange: (body.meta && body.meta.exchange) || null,
      country: (body.meta && body.meta.country) || null,
      exchangeTimezone: (body.meta && body.meta.exchange_timezone) || null,
